...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == STATE_BUDGET:
        ...
        return await update.message.reply_text("🧠 Какие у тебя навыки или интересы? (через запятую)", parse_mode="Markdown")
    ...
    if state == STATE_SKILLS:
        ...
        return await update.message.reply_text("⏱ Сколько времени готов уделять в неделю?", reply_markup=kb)
    ...
    if "@" in text and "." in text:
        ...
        with open("pro_requests.csv", "a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow([datetime.datetime.utcnow().isoformat(), update.effective_user.id, update.effective_user.username, text])
        return await update.message.reply_text("✅ Супер! Ты в списке ожидания PRO-версии 📬.")
